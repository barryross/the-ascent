$(document).ready(() => {
	...
	mobileNav();
		...
		//Passes in a parent list item node, and a boolean value to indicate whether that 
		function showSubMenu(li){
			...
			li.find("button.nav-arrow").attr("aria-expanded", "true")
		...
		function closeSubMenu(li){
			...
			li.find("button.nav-arrow").attr("aria-expanded", "false")
		}

		$(".nav").on('blur', 'li', function(evt){
			if($(this).is(':last-child')) $(this).closest(".open").removeClass("open");
		});
		...
		$("li.nav-menu__item > a, button.nav-arrow").mouseover(function(){
			var $a = $(this)
			if(!$a.parent().hasClass("open")){ showSubMenu($a.parent())	}
		})

		$("li.nav-menu__item").mouseleave(function(){
			var $li = $(this);
			if($li.hasClass("open")){closeSubMenu($li)}
		})
});
